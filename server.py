...
# =========================
# MYSQL CONFIG (Same as pc_processor.py)
# =========================
MYSQL_CONFIG = {
    "host": "147.93.31.4",
    "port": 3306,
    "user": "root",
    "password": "elitecodo_pw",
    "database": "elitecodo_db"
}
...
# ================================
# Masters read-only API (MySQL - Same credentials as pc_processor.py)
# ================================
@app.route('/api/masters_mysql', methods=['GET'])
def get_masters_mysql():
    """Fetch master data from MySQL database using same credentials as pc_processor.py"""
    conn = None
    try:
        ...
        def fetch(table, id_col, name_col):
            """Fetch data from a master table"""
        ...
        data = {
            ...
            "levels": fetch('question_level', 'question_level_id', 'question_level'),
            "exams": fetch('entrance_exam_master', 'entrance_exam_id', 'entrance_exam_name'),
            "patterns": fetch('pattern_master', 'pattern_id', 'pattern_name'),
            "types": fetch('question_type', 'question_type_id', 'question_type'),
            "years": fetch('year_of_appearance', 'year_of_appearance_id', 'year_of_appearance'),
            "topics": fetch('topic_master', 'topic_id', 'topic_name'),
            "sub_topics": fetch('sub_topic_master', 'sub_topic_id', 'sub_topic_name'),
            "users": fetch('users', 'id', 'username'),
        }
        return jsonify(data)
...
@app.route('/api/chapters_by_subject_mysql/<int:subject_id>', methods=['GET'])
def get_chapters_by_subject_mysql(subject_id):
    """Get chapters by subject from MySQL database"""
    conn = None
    try:
        conn = mysql.connector.connect(**MYSQL_CONFIG)
        cur = conn.cursor(dictionary=True)
        ...
        cur.execute("""
            ...
            FROM chapter_master 
            WHERE subject_master_subject_id = %s 
        ...
        print(f"❌ MySQL chapters query error: {e}")
...
def serve_uploaded_file(filename):
    """Serve uploaded files including LaTeX images"""
    return send_from_directory(app.config['UPLOAD_FOLDER'], filename)


@app.route('/api/upload_latex_image', methods=['POST'])
def upload_latex_image():
    """Upload LaTeX image to server"""
    try:
        if 'file' not in request.files:
            return jsonify({'error': 'No file provided'}), 400
        
        file = request.files['file']
        directory = request.form.get('directory', 'latex_images')
        ...
        if file and file.filename.lower().endswith('.png'):
            ...
            image_url = f"/uploads/{directory}/{filename}"
